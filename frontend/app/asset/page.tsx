    ...
    <div style={{ padding: "24px", height: "100%", overflowY: "auto" }}>
      ...
      {tab === "procedure" && (
        <div>
            ...
            <div style={{ background: "var(--bg-elevated)", border: "1px solid var(--blue)", borderRadius: "10px", padding: "16px", marginBottom: "16px" }}>
              ...
              <div style={{ display: "flex", gap: "8px", justifyContent: "flex-end" }}>
                ...
                <button style={btnPrimary} disabled={procSaving} onClick={async () => {
                  ...
                  setProcSaving(true);
                  try {
                    await apiPost(`/assets/${assetId}/procedure`, {
                      titolo: procNuovaForm.titolo.trim(),
                      tipo: procNuovaForm.tipo,
                      passi: procNuovaForm.passiRaw.split("\n").map(s => s.trim()).filter(Boolean),
                    });
                    setProcNuovaForm({ show: false, titolo: "", tipo: "ispezione", passiRaw: "" });
                    await loadProcedure();
                  } catch { } finally { setProcSaving(false); }
                }}>
                  {procSaving ? "Salvo..." : "Crea"}
                </button>
              </div>
            </div>
          )}
          ...
          {procedure.map(proc => {
            const tipoColor = { ispezione: "#3b82f6", sostituzione: "#f59e0b", taratura: "#6b7280", loto: "#ef4444", emergenza: "#f97316" }[proc.tipo] ?? "#3b82f6";
            const isEditing = procEditId === proc.id;
            const isExpanded = procExpanded.has(proc.id);
            return (
              <div key={proc.id} style={{ background: "var(--bg-elevated)", border: `1px solid ${tipoColor}44`, borderLeft: `3px solid ${tipoColor}`, borderRadius: "8px", marginBottom: "10px", overflow: "hidden" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 14px", cursor: "pointer" }}
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    ...
                    <span style={{ fontSize: "11px", color: "var(--text-secondary)" }}>rev.{proc.revisione} · {proc.passi.length} passi</span>
                  </div>
                ...
                {isEditing && (
                  <div style={{ padding: "12px 14px", borderTop: "1px solid var(--border)", background: "var(--bg-surface)" }}>
                    <FormField label="Titolo">
                      <input style={inputStyle} value={procEditForm.titolo} onChange={e => setProcEditForm(p => ({ ...p, titolo: e.target.value }))} />
                    </FormField>
                    <FormField label="Tipo">
                    ...
                    <div style={{ display: "flex", gap: "8px", justifyContent: "flex-end" }}>
                      ...
                      <button style={btnPrimary} disabled={procSaving} onClick={async () => {
                        setProcSaving(true);
                        try {
                          await apiPut(`/assets/${assetId}/procedure/${proc.id}`, { titolo: procEditForm.titolo, tipo: procEditForm.tipo, passi: procEditForm.passiRaw.split("\n").map(s => s.trim()).filter(Boolean) });
                          setProcEditId(null);
                          await loadProcedure();
                        } catch { } finally { setProcSaving(false); }
                      }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: "8px", justifyContent: "flex-end", marginTop: "10px" }}>
                ...
                <button style={btnPrimary} onClick={async () => {
                  ...
                  try {
                    await apiPut(`/assets/${assetId}/nota-senior`, { testo: notaTesto.trim() });
