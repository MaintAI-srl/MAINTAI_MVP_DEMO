...
function ModalRicambio({
  ...
  const save = async () => {
    if (!form.codice.trim()) { notify.warning(tn("Il codice è obbligatorio")); return; }
    if (!form.descrizione.trim()) { notify.warning(tn("La descrizione è obbligatoria")); return; }
    setSaving(true);
    try {
      if (isEdit) {
        ...
        notify.success(tn("Ricambio creato"));
      }
      onSaved();
      onClose();
    } catch (e) {
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, padding: "16px" }} onClick={onClose}>
      <div style={{ background: "var(--surface-1)", border: "1px solid var(--border-default)", borderRadius: "14px", padding: "24px", width: "100%", maxWidth: "560px", maxHeight: "90vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
        <h2 style={{ margin: "0 0 18px", fontSize: "18px", fontWeight: 800, color: "var(--text-primary)" }}>
          {isEdit ? tr("Modifica ricambio") : tr("Nuovo ricambio")}
        </h2>
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            <Field label="Codice *"><input style={inputSt} value={form.codice} onChange={upd("codice")} placeholder={tr("RIC-001")} /></Field>
            <Field label="Categoria"><input style={inputSt} value={form.categoria} onChange={upd("categoria")} placeholder={tr("Cuscinetti")} /></Field>
          </div>
          <Field label="Descrizione *"><input style={inputSt} value={form.descrizione} onChange={upd("descrizione")} placeholder={tr("Cuscinetto SKF 6205")} /></Field>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            ...
            <Field label="Fornitore"><input style={inputSt} value={form.fornitore} onChange={upd("fornitore")} placeholder={tr("RS Components")} /></Field>
          </div>
          <Field label="Ubicazione"><input style={inputSt} value={form.ubicazione} onChange={upd("ubicazione")} placeholder={tr("Scaffale A-12")} /></Field>
          <Field label="Note"><textarea style={{ ...inputSt, resize: "vertical", minHeight: "56px" }} value={form.note} onChange={upd("note")} /></Field>
          {isEdit && <div style={{ fontSize: "12px", color: "var(--text-muted)" }}>{tr("La giacenza si modifica solo tramite i movimenti di magazzino (carico/scarico).")}</div>}
        </div>
        <div style={{ display: "flex", gap: "10px", justifyContent: "flex-end", marginTop: "20px" }}>
          <button style={btnSec} onClick={onClose}>{tr("Annulla")}</button>
          <button style={btnPri} onClick={save} disabled={saving}>{saving ? "Salvo..." : (isEdit ? tr("Salva modifiche") : tr("Crea ricambio"))}</button>
        </div>
      </div>
    </div>
  );
...
function ModalMovimenti({
  ...
  const save = async () => {
    ...
    if (q === null || q < 0) { notify.warning(tn("Quantità non valida")); return; }
    setSaving(true);
    try {
      await apiPost(`/ricambi/${ricambio.id}/movimento`, { tipo, quantita: q, causale: causale.trim() || null });
      notify.success(tn("Movimento registrato"));
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, padding: "16px" }} onClick={onClose}>
      <div style={{ background: "var(--surface-1)", border: "1px solid var(--border-default)", borderRadius: "14px", padding: "24px", width: "100%", maxWidth: "560px", maxHeight: "90vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
        <h2 style={{ margin: "0 0 4px", fontSize: "18px", fontWeight: 800, color: "var(--text-primary)" }}>{tr("Movimenti ·")} {ricambio.codice}</h2>
        <div style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 16 }}>
          {tr("Giacenza")} {ricambio.giacenza} · Prenotato {ricambio.prenotato} · Disponibile <strong style={{ color: ricambio.disponibile > 0 ? "#22c55e" : "#ef4444" }}>{ricambio.disponibile}</strong>
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
          <Field label="Tipo movimento">
            <select style={inputSt} value={tipo} onChange={e => setTipo(e.target.value as typeof tipo)}>
              <option value="carico">{tr("Carico (+)")}</option>
              <option value="scarico">{tr("Scarico (−)")}</option>
              <option value="rettifica">{tr("Rettifica inventario (=)")}</option>
            </select>
          </Field>
        ...
        <div style={{ marginTop: 12 }}>
          <Field label="Causale"><input style={inputSt} value={causale} onChange={e => setCausale(e.target.value)} placeholder={tr("Es. rifornimento fornitore / uso su ticket")} /></Field>
        </div>
        <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 14 }}>
          <button style={btnPri} onClick={save} disabled={saving}>{saving ? "Registro..." : tr("Registra movimento")}</button>
        </div>

        <div style={{ marginTop: 20, borderTop: "1px solid var(--border-default)", paddingTop: 14 }}>
          <div style={{ fontSize: 12, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 8 }}>{tr("Storico movimenti")}</div>
          {movimenti.length === 0 && <div style={{ fontSize: 13, color: "var(--text-muted)" }}>{tr("Nessun movimento registrato.")}</div>}
          {movimenti.map(m => (
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 16 }}>
          <button style={btnSec} onClick={onClose}>{tr("Chiudi")}</button>
        </div>
      </div>
    </div>
  );
...
export default function MagazzinoPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      notify.error(tn("Errore nel caricamento dei ricambi"));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const del = async (r: Ricambio) => {
    if (!confirm(tn("Disattivare il ricambio \"{codice}\"? Lo storico viene conservato.", { codice: r.codice }))) return;
    try {
      await apiDelete(`/ricambi/${r.id}`);
      notify.success(tn("Ricambio disattivato"));
      await load();
    } catch {
      notify.error(tn("Errore nella disattivazione"));
    }
  };
  ...
  return (
    <div style={{ padding: "24px", maxWidth: "1200px", margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "20px", flexWrap: "wrap", gap: "12px" }}>
        <div>
          <h1 style={{ margin: 0, fontSize: "24px", fontWeight: 800, color: "var(--text-primary)" }}>{tr("Magazzino Ricambi")}</h1>
          <div style={{ fontSize: "14px", color: "#64748b", marginTop: "4px" }}>
            {tr("{articoli} articoli · {scorta} sotto scorta · valore stimato € {valore}", { articoli: ricambi.length, scorta: sottoScortaCount, valore: totValore.toLocaleString(getLocaleTag(), { maximumFractionDigits: 0 }) })}
          </div>
        </div>
        <button style={btnPri} onClick={() => setModalRicambio("new")}>{tr("+ Nuovo ricambio")}</button>
      </div>

      <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
        <input style={{ ...inputSt, maxWidth: 320 }} value={q} onChange={e => setQ(e.target.value)} placeholder={tr("Cerca per codice, descrizione o categoria…")} />
        <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "var(--text-soft)", cursor: "pointer" }}>
          ...
          {tr("Solo sotto scorta")}
        </label>
      </div>

      {loading && <div style={{ textAlign: "center", padding: "60px", color: "var(--text-muted)" }}>{tr("Caricamento...")}</div>}
      ...
      {!loading && filtered.length === 0 && (
        <div style={{ textAlign: "center", padding: "60px", background: "var(--surface-2)", borderRadius: "12px", border: "1px solid var(--border-default)" }}>
          ...
          <div style={{ fontSize: "18px", color: "var(--text-muted)", marginBottom: "8px" }}>{tr("Nessun ricambio")}</div>
          <button style={btnPri} onClick={() => setModalRicambio("new")}>{tr("Aggiungi il primo ricambio")}</button>
        </div>
      )}
      ...
      {!loading && filtered.length > 0 && (
        <div style={{ background: "var(--surface-2)", borderRadius: "12px", border: "1px solid var(--border-default)", overflow: "auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1.2fr 2fr 1fr 0.8fr 0.8fr 0.8fr 1fr 96px", gap: "10px", padding: "12px 16px", borderBottom: "1px solid var(--border-default)", fontSize: "11px", color: "#64748b", textTransform: "uppercase", letterSpacing: "0.5px", minWidth: 900 }}>
            <div>{tr("Codice")}</div><div>{tr("Descrizione")}</div><div>{tr("Categoria")}</div>
            <div style={{ textAlign: "right" }}>{tr("Giacenza")}</div>
            <div style={{ textAlign: "right" }}>{tr("Prenotato")}</div>
            <div style={{ textAlign: "right" }}>{tr("Disponibile")}</div>
            <div>{tr("Ubicazione")}</div><div></div>
          </div>
          {filtered.map(r => (
            <div key={r.id} style={{ display: "grid", gridTemplateColumns: "1.2fr 2fr 1fr 0.8fr 0.8fr 0.8fr 1fr 96px", gap: "10px", padding: "12px 16px", borderBottom: "1px solid var(--border-default)", alignItems: "center", fontSize: "14px", minWidth: 900, opacity: r.attivo ? 1 : 0.5 }}>
              <div style={{ fontWeight: 700, color: "var(--text-primary)" }}>
                {r.codice}
                {r.sotto_scorta && <span title={tr("Sotto scorta minima")} style={{ marginLeft: 6, background: "#7f1d1d", color: "#fca5a5", borderRadius: 4, padding: "1px 6px", fontSize: 10, fontWeight: 800 }}>{tr("SCORTA")}</span>}
              </div>
              ...
              <div style={{ display: "flex", gap: 4, justifyContent: "flex-end" }}>
                <button onClick={() => setModalMovimenti(r)} title={tr("Movimenti magazzino")} style={{ background: "transparent", border: "none", cursor: "pointer", fontSize: 16, padding: "2px 4px" }}>📊</button>
                <button onClick={() => setModalRicambio(r)} title={tr("Modifica")} style={{ background: "transparent", border: "none", cursor: "pointer", fontSize: 15, padding: "2px 4px" }}>✏️</button>
                <button onClick={() => del(r)} title={tr("Disattiva")} style={{ background: "transparent", border: "none", cursor: "pointer", color: "#64748b", fontSize: 15, padding: "2px 4px" }}>🗑</button>
              </div>
            </div>
          ))}
        </div>
