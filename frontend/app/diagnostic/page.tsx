...
function DiagnosticContent() {
  ...
  async function sendReply() {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { from: "ai", step: { type: "question", content: "Errore di connessione. Riprova.", detail: "" }, timestamp: new Date() },
      ]);
    } finally {
      setSending(false);
    }
  ...
  return (
    <div style={styles.root}>
      ...
      <main style={styles.main}>
        {status === "idle" && (
          <div style={styles.startScreen}>
            ...
            <button style={styles.startBtn} onClick={startSession}>AVVIA SESSIONE</button>
          </div>
        )}
        {status === "loading" && (
          <div style={styles.startScreen}>
      ...
      {status === "concluded" && (
        <footer style={styles.footer}>
          <div style={styles.concludedBar}>✓ SESSIONE CONCLUSA — ROOT CAUSE IDENTIFICATA</div>
        </footer>
      )}
    </div>
  );
...
const styles: Record<string, React.CSSProperties> = {
  ...
  gridBg: { position: "fixed", inset: 0, backgroundImage: `linear-gradient(rgba(255,255,255,0.02) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.02) 1px, transparent 1px)`, backgroundSize: "40px 40px", pointerEvents: "none", zIndex: 0 },
  header: { position: "relative", zIndex: 1, display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 24px", borderBottom: "1px solid #1f2937", background: "var(--bg-surface)" },
  ...
  spinner: { width: "32px", height: "32px", border: "2px solid #1f2937", borderTop: "2px solid #6b7280", borderRadius: "50%", animation: "spin 1s linear infinite" },
  ...
  bubbleDetail: { fontSize: "12px", lineHeight: 1.5, margin: 0, color: "var(--text-muted)", borderLeft: "2px solid #1f2937", paddingLeft: "10px" },
  ...
  footer: { position: "relative", zIndex: 1, borderTop: "1px solid #1f2937", background: "var(--bg-surface)", padding: "16px 24px" },
