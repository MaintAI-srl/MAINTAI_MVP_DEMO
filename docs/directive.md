...
### 11. Il Piano di Manutenzione è Persistente, Multi-Asset e Automatico
Il termine "Piano di Manutenzione" fa riferimento alla tabella `piani_manutenzione`. Usa questa vera entità a database. Il sistema supporta l'associazione Multi-Asset (Many-to-Many). Deve autogenerare progressivi e nomi codificati (es. PM-2026-001) se non forniti. I ticket creati via import (PDF AI/OCR o Excel) devono essere agganciati al piano con `origine_piano` coerente. I manuali sono ora integrati nel contesto del piano tramite `piano_id`.
