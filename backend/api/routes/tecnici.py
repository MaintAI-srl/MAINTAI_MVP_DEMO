...
def _working_days_in_range(start: date, end: date) -> int:
    """Conta giorni lavorativi lun-ven nell'intervallo [start, end]."""
    count = 0
    current = start
    while current <= end:
...
@router.get("/tecnici")
def get_tecnici(db: Session = Depends(get_db)):
    return tecnico_repository.get_all(db)


@router.get("/tecnici/disponibili")
def get_tecnici_disponibili(db: Session = Depends(get_db)):
    """Solo tecnici con stato='in servizio'."""
    return tecnico_repository.get_disponibili(db)


@router.get("/tecnici/me")
def get_current_tecnico(db: Session = Depends(get_db), utente_id: int = Query(...)):
    """Restituisce il profilo del tecnico collegato all'utente loggato."""
    from backend.db.modelli import Tecnico
    tecnico = db.query(Tecnico).filter(Tecnico.utente_id == utente_id).first()
    if not tecnico:
...
def tecnici_statistiche(db: Session = Depends(get_db)):
    """
    Ore uomo disponibili calcolate su calendario lavorativo lun-ven.
    Esclude tecnici non in servizio (ferie, malattia, corso).
    """
    today = date.today()
    attivi = tecnico_repository.get_disponibili(db)
    ore_giornaliere_totali = sum(t["ore_giornaliere"] for t in attivi)

    # Giornaliero
    daily = ore_giornaliere_totali if today.weekday() < 5 else 0

    # Settimanale (lun-ven della settimana corrente)
    week_start = today - timedelta(days=today.weekday())
    week_end = week_start + timedelta(days=4)
    wd_week = _working_days_in_range(max(today, week_start), week_end)
    weekly = ore_giornaliere_totali * wd_week

    # Mensile (giorni lavorativi restanti nel mese corrente)
    last_day = date(today.year, today.month, monthrange(today.year, today.month)[1])
    wd_month = _working_days_in_range(today, last_day)
    monthly = ore_giornaliere_totali * wd_month

    # Annuale (giorni lavorativi restanti nell'anno corrente)
...
def create_tecnico(tecnico: TecnicoCreate, db: Session = Depends(get_db)):
    ...
    return tecnico_repository.create(db, tecnico)


@router.put("/tecnici/{tecnico_id}")
def update_tecnico(tecnico_id: int, data: TecnicoUpdate, db: Session = Depends(get_db)):
    if data.stato and data.stato not in STATI_VALIDI:
        raise HTTPException(status_code=422, detail=f"Stato non valido. Valori ammessi: {', '.join(STATI_VALIDI)}")
    updated = tecnico_repository.update(db, tecnico_id, data)
    if not updated:
...
def delete_tecnico(tecnico_id: int, db: Session = Depends(get_db)):
    ok = tecnico_repository.delete(db, tecnico_id)
    if not ok:
        raise HTTPException(status_code=404, detail="Tecnico non trovato")
...
def get_assenze_tecnico(tecnico_id: int, db: Session = Depends(get_db)):
...
def create_assenza_tecnico(tecnico_id: int, assenza: TecnicoAssenzaCreate, db: Session = Depends(get_db)):
...
def delete_assenza_tecnico(assenza_id: int, db: Session = Depends(get_db)):
